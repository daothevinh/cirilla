...
class _ProductScreenState extends State<ProductScreen>
  ...
  int? _getQty(Product? product) {
    ...
    return product?.stockQuantity ?? 9999;
  }
    ...
    return Observer(builder: (_) {
      ...
      Widget? bottomBar = enableBottomBar
          ? ProductBottomBar(
              ...
              qty: ProductQuantity(
                max: _getQty(productData) != null && _getQty(productData)! > 0 ? _getQty(productData) : 9999,
                qty: _qty,
                onChanged: (int value) {
                  setState(() {
                    _qty = value;
  ...
  Widget buildBlock(
    ...
    int? maxQty = _getQty(productOrVariableProduct) != null && _getQty(productOrVariableProduct)! > 0
        ? _getQty(productOrVariableProduct)
        : 9999;
    switch (type) {
      ...
      case ProductBlocks.sortDescription:
        return ProductSortDescription(product: productOrVariableProduct);
