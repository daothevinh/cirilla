...
class CartScreenState extends State<CartScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (_) {
      if (_items != _cartStore?.cartData?.items) {
        _items = _cartStore?.cartData?.items;
      }
      enableKey = !_cartStore!.loadingAddCart;
      return Scaffold(
        appBar: buildAppBar(translate: translate, textTheme: textTheme),
        body: Stack(
          children: [
            if (_cartStore?.cartData != null) ...[
            ...
            if (_items?.isEmpty == true) buildCartEmpty(context, textTheme),
  ...
  AppBar buildAppBar({
    ...
    return AppBar(
      actions: _cartStore?.cartData == null || _items!.isEmpty
          ? null
          : [
              Padding(
  ...
  Widget buildContent({
    ...
    return CustomScrollView(
      slivers: <Widget>[
        SliverAnimatedList(
          key: enableKey ? _listKey : Key('${_items!.length}'),
      ...
      child: Column(
        ...
        children: [
          ...
          if (enableShipping == true && _cartStore?.cartData?.needsShipping == true) ...[
            ...
            CartShipping(
              cartData: _cartStore!.cartData!,
              cartStore: _cartStore,
            ),
            const SizedBox(height: itemPaddingLarge),
          ],
          CartTotal(cartData: _cartStore!.cartData!),
