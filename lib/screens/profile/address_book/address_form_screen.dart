...
class _AddressFormScreenState extends State<AddressFormScreen> with SnackMixin, AppBarMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(
      builder: (_) {
        bool loading = _addressDataStore?.address?.billing?.isNotEmpty != true;

        Map<String, dynamic> addressFields = widget.type == 'shipping'
            ? {'shipping_address_nickname': fieldNickName, ..._addressDataStore?.address?.shipping ?? {}}
            : {'billing_address_nickname': fieldNickName, ..._addressDataStore?.address?.billing ?? {}};
        return Scaffold(
