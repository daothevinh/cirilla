...
class _TabsState extends State<Tabs> with Utility {
  ...
  Widget build(BuildContext context) {
    ...
    double? padTop =
        ConvertData.stringToDouble(get(widgetConfig.styles, ['padTop'], 12));
    double? pad =
        ConvertData.stringToDouble(get(widgetConfig.styles, ['pad'], 4));
    double? padBottom =
        ConvertData.stringToDouble(get(widgetConfig.styles, ['padBottom'], 12));
    ...
    Map<String, dynamic> configStyle =
        enableCustomize ? get(item, ['data'], {}) : widgetConfig.styles;

    Color background = ConvertData.fromRGBA(
        get(configStyle, ['background', themeModeKey], {}),
        theme.bottomAppBarTheme.color);

    Color textColor = ConvertData.fromRGBA(
        get(configStyle, ['color', themeModeKey], {}), Colors.black);

    Color activeColor = ConvertData.fromRGBA(
        get(configStyle, ['colorActive', themeModeKey], {}),
        theme.primaryColor);

    Color colorOnActive = ConvertData.fromRGBA(
        get(configStyle, ['colorOnActive', themeModeKey], {}),
        theme.primaryColor);

    double? radius =
        ConvertData.stringToDouble(get(configStyle, ['radius'], 0));

    double? activeBorderRadius = ConvertData.stringToDouble(
        get(configStyle, ['activeBorderRadius'], 30));
